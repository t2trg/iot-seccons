...
Another promising and similar direction is the usage of some type of equiment -- monitoring device -- that automatically learns and monitors the behaviour of the different IoT devices in a given network. Such monitoring device can be configured by the user to limit traffic and trigger alarms when an unexpected operation is detected. MUD files can be easily embedded in the operation of such monitoring devices.
...
Similarly, many IoT devices that are being deployed today will remain operational for a decade or even longer. During this time, digital signatures used to sign software updates might become obsolete making infeasible the secure update of IoT devices.
