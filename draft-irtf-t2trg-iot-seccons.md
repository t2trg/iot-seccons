...
docname: draft-irtf-t2trg-iot-seccons-07
cat: info
stand_alone: true
informative:
...
wide variety of technologies, objects and protocols. It is not surprising that the IoT has received significant attention from the research community to (re)design, apply, and use standard Internet technology and protocols for the IoT.
...
The introduction of IPv6 {{RFC6568}} and CoAP {{RFC7252}} as fundamental building blocks for IoT applications allows connecting IoT hosts to the Internet. This brings several advantages including: (i) a homogeneous protocol ecosystem that allows simple integration with other Internet hosts; (ii) simplified development for devices that significantly vary in their capabilities; (iii) a unified interface for applications, removing the need for application-level proxies. These building blocks greatly simplify the deployment of the envisioned scenarios which range from building automation to production environments and personal area networks.
...
Symmetric and asymmetric keys can be used in group communication. Asymmetric keys have the key advantage that they ensure source authentication. However, doing broadcast encryption with a single public- private key pair is also not feasible. The usage of a single symmetric key can be used to encrypt, but it has inherent risks since the capture of a single node compromises the key. The usage of symmetric-keys also does not provide source authentication. It is also a well established fact that asymmetric cryptography is more expensive than symmetric solutions. Thus, the security risks and performance trade-offs of applying either symmetric or asymmetric keys to a given IoT use case need to be well-analyzed according to risk and usuability assessments.
...
Software updates are required not only to add new functionality to IoT devices but also to eliminate new security vulnerabilities due to software bugs, design flaws, or deprecated algoritms.
...
Flaws in the design and implementation of a secure IoT device and network can lead to security vulnerabilities. For instance, a flaw is the distribution of an Internet-connected IoT device in which a default password is used in all devices. Many IoT devices can be found in the Internet by means of tools such as Shodan {{shodan}}, and if they have any vulnerability, it can then be exploited at scale, for example, to launch DDoS attacks. For instance, Dyn, a mayor DNS, was attacked by means of a DDoS attack originated from a large IoT botnet composed of thousands of compromised IP-cameras {{dyn-attack}}. Open questions in this area are:
