...
informative:
  ...
  WG-TEEP:
    title: IETF Trusted Execution Environment Provisioning (teep) 
    seriesinfo:
      Web: https://datatracker.ietf.org/wg/teep/about/
  WG-MSEC:
  ...
  venona-project:
    ...
    seriesinfo:
      Web: https://www.nsa.gov/news-features/declassified-documents/venona/index.shtml
  LWM2M:
    title: OMA LWM2M
    seriesinfo:
  ...
  DCMS:
    ...
    seriesinfo:
      Web: https://www.gov.uk/government/publications/secure-by-design    
  OWASP:
  ...
  NISTSP800-34r1:
    title: NIST SP800-34r1 - Contingency Planning Guide for Federal Information Systems
    author:
    - ins: Marianne Swanson
    - ins: Pauline Bowen
    - ins: Amy Wohl Phillips
    - ins: Dean Gallup 
    - ins: David Lynes 
    seriesinfo:
      Web: https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-34r1.pdf
  NISTSP800-30r1:
    title: NIST SP 800-30r1 - Guide for Conducting Risk Assessments
    seriesinfo:
      Web: https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-30r1.pdf
  NISTSP800-122:
    title: NIST SP800-122 - Guide to Protecting the Confidentiality of Personally Identifiable Information
    author:
    - ins: Erika McCallister
    - ins: Tim Grance
    - ins: Karen Scarfone
    seriesinfo:
      Web: https://nvlpubs.nist.gov/nistpubs/legacy/sp/nistspecialpublication800-122.pdf
  SchneierSecurity:
...
1. Vulnerable Software/Code: Things in the Internet of Things rely on software that might contain severe bugs and/or bad design choices. This makes the things vulnerable to many different types of attacks, depending on the criticality of the bugs and converts software vulnerabilies as one of the most important security threats. For instance, a Mirai botnet allows for a coordinated large-scale distributed denial-of-service (DDoS) on one or multiple targets.
The coordinator of the attack deployes a self-propagating botnet virus that is capable of both scanning for new potential targets to compromise (so that they become part of the botnet) and performing DoD attacks on a target as specified by the control center. When a compromised device discovers a potentially vulnerable device, e.g., reusing default passwords or lacking proper authentication, it will inform the control center that will proceed to infect it further augmenting its infection and attack capabilities.
...
The way a BIA, RA, PIA are exactly performed depends on the environment and industry. 
We refer the readers to NIST documents such as {{NISTSP800-34r1}}, {{NISTSP800-30r1}}, or {{NISTSP800-122}}.
...
IoT deployments are often characterized by lossy and low-bandwidth communication channels. IoT devices are also often constrained in terms of CPU, memory, and energy budget available {{RFC7228}}. These characteristics directly impact the threats to and the design of protocols for the IoT domain. For instance, small packet size limits at the physical layer (127 Bytes in IEEE 802.15.4) can lead to (i) hop-by-hop fragmentation and reassembly or (ii) lead to small IP-layer MTU sizes. In the first case, fragmentation of larger packets required by security protocols may open new attack vectors for state exhaustion DoS attacks, which is especially tragic, for example, if the fragmentation is caused by large key exchange messages. In the second case, this might lead to more fragmentation at IP layer, that commonly downgrades the overall system performance due to fragment losses and the need for retransmissions, e.g., fate-sharing packet flight as implemented by DTLS might aggravate the resulting performance loss.
...
4. Homomorphic encryption techniques can be used in the middlebox to perform certain operations. However, this is limited to data processing involving arithmetic operations. Furthermore, performance of existing libraries, for example, SEAL {{SEAL}} is still too limited and SEAL, as an example, and homomorphic encryption, in general, are is not widely applicable yet.
...
The above scenario becomes more urgent when we consider the so called "harvest and decrypt" attack in which an attacker can start to harvest (store) encrypted data today, before a quantum-computer is available, and decrypt it years later, once a quantum computer is available. Such an attack pattern is not new (for instance, see {{venona-project}}) and it will be likely used once quantum computers  are available.
